...
**神经图学习** ( **NGL** )基本上概括了之前的公式，并且，正如我们将看到的，使得无缝地将图正则化应用于任何形式的神经网络成为可能，包括CNN 和**递归神经网络** ( **RNNs** )。特别是，有一个非常强大的框架，名为**神经结构化学习** ( **NSL** )，它允许我们用很少的几行代码扩展一个在 TensorFlow 中实现的神经网络，并使用图正则化。网络可以是任何类型的:天然的或合成的。
