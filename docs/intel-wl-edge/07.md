...
除了针对主键或组合键的查询之外，**辅助索引**允许用户使用不同的键查询表中的数据。这使您的应用程序在查询不同用例的数据时更加灵活。使用辅助索引执行查询非常类似于直接从表中查询。
...
如您所见，数据流中有多个层通常使用大数据世界中的**提取、转换和加载** ( **ETL** )方法或**提取、加载和转换** ( **ELT** )方法来实现。ETL 方法包括从不同来源提取数据、实现数据质量和一致性标准、转换(或聚合)数据以符合标准格式，以及将数据加载(或交付)到下游应用程序的步骤。
