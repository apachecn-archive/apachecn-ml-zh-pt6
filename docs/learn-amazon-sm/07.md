...
几年来，CV 风靡全球，每个月都有从图像和视频中提取模式的新突破。在这一章中，你将学习三个专门为 CV 任务设计的内置算法。我们将讨论您可以用它们解决的问题类型。我们还将花大量时间解释如何准备图像数据集，因为这个至关重要的话题经常被莫名其妙地忽略。当然，我们也会培训和部署模型。
...
现在，让我们准备Pascal VOC 数据集。该数据集经常用于基准对象检测和语义分割模型:
...
这要简单得多，不是吗？`im2rec`有附加选项来调整图像大小等。它还可以将数据集分成几个块，这是一种有用的技术用于**管道模式**和**分布式训练**。我们将在第 9 章 、*调整你的培训工作*中学习它们。
...
在本节中，我们将使用公共图像数据集，使用所有三种算法来训练和部署模型。我们将涵盖从头开始的培训和迁移学习。
...
4.  We launch the training job as follows:
    ...
    在培训日志中，我们看到数据下载大约需要 3 分钟。惊喜，惊喜:我们还看到算法在训练前构建 RecordIO 文件。此步骤持续约 1 分钟:
...
在这种情况下，从头开始培训根本不是一个选项。相反，我们将使用一种称为**迁移学习**的技术，其中我们从一个已经在非常大和多样化的图像数据集上训练过的网络开始。**ImageNet**([http://www.image-net.org/](http://www.image-net.org/))大概是预训练最受欢迎的选择，有 1000 个类，几百万张图片。
...
5.  我们配置频道并启动培训工作。该代码与上例中的*步骤 4* 完全相同。
...
6.  训练持续 1 小时 40 分钟。这是一个相当重的模型！我们得到的**平均精度度量** ( **图**)为 0.5151。生产使用将需要更多的培训，但我们应该已经能够测试模型。
...
4.  我们配置了四个通道，将源图像的内容类型设置为`image/jpeg`，将遮罩图像的内容类型设置为`image/png`。然后，我们启动培训作业:
...
如你所见，这三种算法使得训练 CV 模型变得很容易。即使使用默认的超参数，我们也能很快得到好的结果。尽管如此，我们开始觉得有必要扩大我们的培训工作。不要担心，一旦相关的特性在以后的章节中被涉及到，我们将会重温一些我们的 CV 例子，我们将会从根本上扩展它们！
