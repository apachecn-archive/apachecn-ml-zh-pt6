...
检测 ML 偏差对于任何 ML 项目的成功都是至关重要的。如果 ML 偏差没有被检测和减轻，利用 ML 模型的自动化系统可能会以不公平的预测而告终。例如，基于 ML 的招聘应用程序可能对某些群体做出不公平的候选人选择(例如，对女性候选人)。另一个例子是一个自动贷款申请，它拒绝来自代表不足的群体(例如，那些生活在特定国家的群体)的贷款申请。
