...
我们还没完呢！此时，我们只完成了 ML 管道的一半。请确保您没有关闭 SageMaker Studio 中正在运行的应用和实例，因为我们稍后将在`Machine Learning Pipelines with SageMaker Pipelines.ipynb`笔记本中运行更多代码块来完成我们的管道。

注意

如果您需要休息，您可以关闭正在运行的实例和应用(以管理成本)，然后在继续进行本章的*完成端到端 ML 管道*部分之前，再次运行`Machine Learning Pipelines with SageMaker Pipelines.ipynb`笔记本中的所有单元。
