...
*   *选项 4* : *在 EC2 实例+ boto3 + SageMaker ML 推断端点*内使用 web 框架(例如 Flask 或 Django)定制基于容器的 web 应用——web 应用(在`boto3`库中的容器内运行以调用 SageMaker ML 推断端点。
...
*   *选项 6* : *自定义基于容器的 web 应用，使用 web 框架(例如，Flask 或 Django)和弹性 Kubernetes 服务(EKS) + boto3 + SageMaker ML 推理端点*——web 应用(运行在`boto3`库内，调用 SageMaker ML 推理端点。
