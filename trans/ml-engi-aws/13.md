...
在这里，我们可以看到，我们还可以指定一个可选的 KMS 密钥，用于加密亚马逊 S3 中的输出文件。除了加密静态数据，我们还需要在执行分布式培训时确保安全的数据传输。当执行培训作业时使用多个实例时，我们可以启用**容器间流量加密**来保护实例间传输的数据。如果我们需要遵守特定的法规要求，我们需要确保传输的数据也是加密的。
